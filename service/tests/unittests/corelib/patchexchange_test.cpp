...
public:
  ...
  MOCK_METHOD(corral::Task<patch::PreparePatchResult>, preparePatch,
              (const std::string &folderPath), (override));
};
...
//                             [](uv_async_t *handle) {
//                               auto data =
//                                   reinterpret_cast<CheckData
//                                   *>(handle->data);
//                               (*data->callback)(
//                                   data->folderPath,
//                                   patch::LocalStateResult{.initialized =
//                                   true});
//                               if (data->thread) {
//                                 data->thread->join();
//                                 delete data->thread;
...
//     ON_CALL(*backend, checkLocalState(_))
...
//               thread = new std::thread([preparePatchResult,
//               &commandRunning]() {
//                 std::this_thread::sleep_for(20ms);
//                 commandRunning = false;
...
//               });
//             });
//     EXPECT_CALL(*backend, checkLocalState("folder1")).Times(1);
//     EXPECT_CALL(*backend, checkLocalState("folder2")).Times(1);
...
//                                  // Check that possible exceptions in
//                                  callbacks
